...
class Test ( name: String, scope: CoroutineScope, isconfined: Boolean=false, isdynamic: Boolean=false ) : 
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		return { //this:ActionBasciFsm
				...
				state("go") { //this:State
					action { //it:State
						CommUtils.outred("$name: now I'll send a message to cargorobot and he has to go")
						forward("go", "go(1)" ,"cargorobot" ) 
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
					sysaction { //it:State
				...
				state("done") { //this:State
					action { //it:State
						CommUtils.outred("$name: my work here is done, see ya")
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
					sysaction { //it:State
